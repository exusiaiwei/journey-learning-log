...
class Command(ScrapyCommand):
    ...
    def run(self, args, opts):
        ...
        print(args)
        print(configLib)
        print(configLib.ConfigClass)
        cfg = configLib.ConfigClass()
        if len(args) >= 3:
        ...
        configLib.init_database(cfg.config['MYSQL_HOST'], cfg.config['MYSQL_USER'], cfg.config['MYSQL_PASSWD'],cfg.config['MYSQL_PORT'], dbname)
        
        log = configLib.log(tbname, dbname, self.settings['BEGIN_PAGE'], opts.good_only, opts.see_lz)
